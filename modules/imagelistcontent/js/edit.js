/*
 * edit.js: Edit functions for image lists
 */
...
function onEditPopupOpen_imagelistcontent(object) {
	$.ajax({
		type:'GET',
		url:'handlers/edit/ajax.php',
		data:{pageid:curPageID,contentArea:object.attr("id"),contentType:'imagelistcontent'},
		async:false
	}).done(function(data){
		// Parse data
...
function valueAssign_imagelistcontent() {
	updateImageListContent();
	return $('#imagelist-changes').val();
}

$(document).on("click",".imagelist-up,.imagelist-down,.imagelist-delete,#imagelist-new-image,#imagelist-add-image",function(){
	...
	} else if($(this).hasClass('imagelist-delete')) {
		row.remove();
	} else if($(this).attr("id") == "imagelist-new-image") {
	...
	} else if($(this).attr("id") == "imagelist-add-image") {
